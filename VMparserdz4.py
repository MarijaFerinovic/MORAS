class Parser:
    ...
    def parseFile(self, filename):
        ...
        print(self._defined_labels)
        print(self._referenced_labels)

        for i in self._referenced_labels:
            if i[0] not in self._defined_labels:
                self._flag = False
                Parser._error("Parser",i[1], f"Nedefinirani label {i[0]}.")
                return ""
    ...
    def _push(self, src, loc:str, n):
        if not loc.isdigit():
            self._flag = False
            Parser._error("Push",n,"Primljen string, a ocekivan integer \"" + src + "\".")
            return ""
    ...
    #   3. n - linija izvornog koda (radi vracanja greske).
    def _pop(self, dst, loc:str, n):
        if not loc.isdigit():
            self._flag = False
            Parser._error("Push",n,"Primljen string, a ocekivan integer \"" + dst + "\".")
            return ""
    ...
    def _label(self, lab, n):
        if lab in self._defined_labels:
            self._flag = False
            Parser._error("Parser", n, f"Duplicirani label {lab}.")
            return ""
        self._defined_labels.append(lab)

        return "(" + self._func + "$" + lab + ")"
    
    def _goto(self, lab, n):
        if lab not in self._referenced_labels:
            self._referenced_labels.append([lab,n])
        return "@" + self._func + "$" + lab + "\n0;JMP"
